...
这里为了支持[Google Play 要求的16 KB](https://developer.android.com/guide/practices/page-sizes?utm_source=chatgpt.com&hl=zh-cn) 页面大小的要求，使用的ndk版本需要在r26 以上。

生成完成后，可以运行下面的命令检测是否符合16 KB 页面大小的要求：

```
{YourNDKPath}\toolchains\llvm\prebuilt\windows-x86_64\bin\llvm-objdump.exe -p {YourSoFilePath} | Select-String -Pattern "LOAD"
```

运行结果中，如果LOAD off 类型的每一行，结尾数字都大于等于2**14，就符合要求。
